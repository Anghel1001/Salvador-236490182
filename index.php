<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Trying Php</title>
</head>
<body>
    <h1>Salvador, Angel Rhyzelle</h1>
    <ul>
        <li><a href="intro.php"> 1. Introduce Yourself</a></li>
        <li><a href="math.php"> 2. Simple Math</a></li>
        <li><a href="rectangle.php"> 3.  Area and Perimeter of a Rectangle</a></li>
        <li><a href="temp_converter.php"> 4. Temperature Coverter</a></li>
        <li><a href="swap_var.php"> 5. Swapping Variable</a></li>
        <li><a href="salary_calc.php"> 6. Salary Calculator</a></li>
        <li><a href="bmi_calc"> 9. BMI Calculator</a></li>
        <li><a href="manipulation"> 8. String Manipulation</a></li>
        <li><a href="grade_sys"> 10. Simple Grading System</a></li>
        <li><a href="currency_converter"> 11. Currency Converter</a></li>
    </ul>

</body>
</html>